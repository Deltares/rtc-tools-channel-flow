...
class OrificeMixin(OptimizationProblem):
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert isinstance(self, GoalProgrammingMixin), (
            "Can only use OrificeMixin when using GoalProgrammingMixin"
        )
    ...
    def path_constraints(self, ensemble_member):
        ...
        for o in self.orifices():
            ...
            constraints.append(
                (
                    ((o.discharge() / (o.area * o.discharge_coefficient)) ** 2)
                    / (2 * g)
                    + o.head()
                    - M * (1 - status),
                    -inf,
    ...
    def priority_completed(self, priority):
        ...
        for o in self.orifices():
            ...
            max_qs[inds] = (
                0.1 * o.q_max
            )  # A reasonable value so we can still when something is wrong
            open_fraction = actual_qs / max_qs

            self.set_timeseries(
                o.symbol + "_fraction_open", Timeseries(times, open_fraction)
            )
