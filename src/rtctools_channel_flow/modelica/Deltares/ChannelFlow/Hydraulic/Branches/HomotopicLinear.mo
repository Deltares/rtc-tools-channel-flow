...
model HomotopicLinear
  ...
  extends Internal.PartialHomotopic(
    nominal_depth = fill(uniform_nominal_depth, n_level_nodes + 1),
    nominal_width = linspace(width_up, width_down, n_level_nodes + 1),
    H(min = H_b)
  );
  // Nominal depth
  ...
  // Array of Widths
  parameter SI.Distance width[n_level_nodes] = (
    linspace(width_up, width_down, n_level_nodes)
  );
  ...
  parameter SI.Position H_b[n_level_nodes] = (
    linspace(H_b_up, H_b_down, n_level_nodes)
  );
equation
