...
partial model PartialHomotopic
  ...
  // Wind stress
  input SI.Stress wind_stress_u(nominal = 1e-1) = 0.0;
  // Wind stress in x (u, easting) direction (= 0 radians, 0 degrees)
  input SI.Stress wind_stress_v(nominal = 1e-1) = 0.0;
  // Wind stress in y (v, northing) direction (= 0.5*pi radians, 90 degrees)
  // Flow
  ...
  // Rotation
  parameter Real rotation_deg = 0.0;
  // Rotation of branch relative to x (u, easting) in degrees
...
protected
  ...
  parameter SI.Angle rotation_rad = Deltares.Constants.D2R*rotation_deg;
  // Conversion to rotation in radians
...
// Compute dQ/dx for advection term
  ...
  for section in 2:n_level_nodes loop
    if (use_inertia and use_convective_acceleration) then
      if use_upwind then
        _dQ_sq_div_Adx[section] = theta*(smooth_switch(Q[section])*(Q[section]^2/(min_divisor + _cross_sectionq[section]) - Q[section - 1]^2/(min_divisor + _cross_sectionq[section - 1]))/_dxq[section - 1] + (1 - smooth_switch(Q[section]))*(Q[section + 1]^2/(min_divisor + _cross_sectionq[section + 1]) - Q[section]^2/(min_divisor + _cross_sectionq[section]))/_dxq[section]) + (1 - theta)*((Q[section + 1]*Q_nominal/(nominal_width[section + 1]*nominal_depth[section + 1]) - Q[section - 1]*Q_nominal/(nominal_width[section - 1]*nominal_depth[section - 1]))/(_dxq[section - 1] + _dxq[section]));
      else
        _dQ_sq_div_Adx[section] = theta*((Q[section + 1]^2/(min_divisor + _cross_sectionq[section + 1]) - Q[section - 1]^2/(min_divisor + _cross_sectionq[section - 1]))/(_dxq[section - 1] + _dxq[section])) + (1 - theta)*((Q[section + 1]*Q_nominal/(nominal_width[section + 1]*nominal_depth[section + 1]) - Q[section - 1]*Q_nominal/(nominal_width[section - 1]*nominal_depth[section - 1]))/(_dxq[section - 1] + _dxq[section]));
      end if;
    else
...
// Note that the equation is formulated without any divisions, to make collocation more robust.
  ...
  for section in 2:n_level_nodes loop
    if use_manning then
      _friction[section] = friction_coefficient^2*(theta*(Q[section]*sqrt(Q[section]^2 + min_abs_Q^2)*(min_divisor + 0.5*(_wetted_perimeter[section] - semi_implicit_step_size*der(_wetted_perimeter[section]) + _wetted_perimeter[section - 1] - semi_implicit_step_size*der(_wetted_perimeter[section - 1])))^(8.0/6.0)/(min_divisor + 0.5*(_cross_section[section] - semi_implicit_step_size*der(_cross_section[section]) + _cross_section[section - 1] - semi_implicit_step_size*der(_cross_section[section - 1])))^(14.0/6.0)) + (1 - theta)*(Q[section]*sqrt(Q_nominal^2 + min_abs_Q^2)*(nominal_depth[section]*2 + nominal_width[section])^(8.0/6.0)/(nominal_width[section]*nominal_depth[section])^(14.0/6.0)));
    else
      _friction[section] = theta*(Q[section]*sqrt(Q[section]^2 + min_abs_Q^2)*(0.5*(_wetted_perimeter[section] - semi_implicit_step_size*der(_wetted_perimeter[section]) + _wetted_perimeter[section - 1] - semi_implicit_step_size*der(_wetted_perimeter[section - 1])))/(min_divisor + friction_coefficient^2*(0.5*(_cross_section[section] - semi_implicit_step_size*der(_cross_section[section]) + _cross_section[section - 1] - semi_implicit_step_size*der(_cross_section[section - 1])))^2)) + (1 - theta)*(Q[section]*sqrt(Q_nominal^2 + min_abs_Q^2)*(nominal_depth[section]*2 + nominal_width[section])/(min_divisor + friction_coefficient^2*(nominal_width[section]*nominal_depth[section])^2));
    end if;
...
// Substance mass balance
  ...
  annotation(
    Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, preserveAspectRatio = true, initialScale = 0.1, grid = {10, 10}), graphics = {Rectangle(visible = true, fillColor = {0, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-60, -20}, {60, 20}})}));
