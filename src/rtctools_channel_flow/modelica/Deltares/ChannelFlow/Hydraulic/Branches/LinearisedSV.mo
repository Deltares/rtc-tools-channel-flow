...
equation
  ...
  (der(_cross_section[1]) + 2 * (Q_relative[2] - Q_relative[1]) / dx - 2 * _QPerpendicular_distribution[1] / dx) * dx = 0;
  for node in 2:n_level_nodes-1 loop
    // Middle heights calculated by mass conservation
    (der(_cross_section[node]) + (Q_relative[node + 1] - Q_relative[node]) / dx - _QPerpendicular_distribution[node] / dx) * dx= 0;
  end for;
  // Boundary mass conservation
  (der(_cross_section[n_level_nodes]) + 2 * (Q_relative[n_level_nodes + 1] - Q_relative[n_level_nodes]) / dx - 2 * _QPerpendicular_distribution[n_level_nodes] / dx) * dx = 0;
