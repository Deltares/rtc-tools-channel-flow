...
equation
  ...
  _higher_level_above_crest = sqrt((sqrt(((HQDown.H - h) - (HQUp.H - h)) ^ 2 + Modelica.Constants.eps) / 2 + ((HQDown.H - h) + (HQUp.H - h)) / 2) ^ 2 + Modelica.Constants.eps) / 2 + ((sqrt(((HQDown.H - h) - (HQUp.H - h)) ^ 2 + Modelica.Constants.eps) / 2 + ((HQDown.H - h) + (HQUp.H - h)) / 2)) / 2;
  //Max[-1*Max[-(HQDown.H-h),-(HQUp.H-h)],0]
  _lower_level_above_crest = sqrt((-1 * (sqrt(((-(HQDown.H - h)) - (-(HQUp.H - h))) ^ 2 + Modelica.Constants.eps) / 2 + ((-(HQDown.H - h)) + (-(HQUp.H - h))) / 2)) ^ 2 + Modelica.Constants.eps) / 2 + (-1 * (sqrt(((-(HQDown.H - h)) - (-(HQUp.H - h))) ^ 2 + Modelica.Constants.eps) / 2 + ((-(HQDown.H - h)) + (-(HQUp.H - h))) / 2)) / 2;
  //Max[(HQUp.H-h),(HQDown.H-h)]-submerged_flow_ratio*-1*Max[-(HQUp.H-h),-(HQDown.H-h)]
  _submerged_flow_test = sqrt(((HQUp.H - h) - (HQDown.H - h)) ^ 2 + Modelica.Constants.eps) / 2 + ((HQUp.H - h) + (HQDown.H - h)) / 2 + submerged_flow_ratio * (sqrt(((-(HQUp.H - h)) - (-(HQDown.H - h))) ^ 2 + Modelica.Constants.eps) / 2 + ((-(HQUp.H - h)) + (-(HQDown.H - h))) / 2);
  ...
  _nonlinear_Q_submerged_weir = width * _lower_level_above_crest * sqrt(2.0 * Modelica.Constants.g_n * submerged_flow_factor *  sqrt((HQUp.H - HQDown.H) ^ 2 + Modelica.Constants.eps)) * (HQUp.H - HQDown.H) / sqrt((HQUp.H - HQDown.H) ^ 2 + Modelica.Constants.eps);
  ...
  _nonlinear_Q_free_weir = 2.0 / 3.0 * width * (2.0 / 3.0 * Modelica.Constants.g_n) ^ 0.5 * _higher_level_above_crest ^ exponent * (HQUp.H - HQDown.H) / sqrt((HQUp.H - HQDown.H) ^ 2 + Modelica.Constants.eps);
  ...
  _one_if_free_flow = (sqrt(_submerged_flow_test ^ 2 + Modelica.Constants.eps) / 2 + _submerged_flow_test / 2) / sqrt(_submerged_flow_test ^ 2 + Modelica.Constants.eps);
