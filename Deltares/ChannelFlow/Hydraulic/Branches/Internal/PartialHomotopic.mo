...
equation
  ...
  for section in 2:n_level_nodes loop
    ...
    // Substance transport
    M[section, :] = theta * (smooth_switch(Q[section]) * (Q[section] .* C[section - 1, :]) + (1 - smooth_switch(Q[section])) * (Q[section] .* C[section, :])) + (1 - theta) * (Q_nominal * C_nominal + C_nominal * (Q[section] - Q_nominal) + Q_nominal * ((C[section - 1, :] + C[section, :]) / 2 - C_nominal));
  end for;
  ...
  for node in 1:n_level_nodes loop
    ...
    theta * der(_cross_section[node] * C[node, :]) + (1 - theta) * (0.5 * (nominal_width[node + 1] * nominal_depth[node + 1] + nominal_width[node] * nominal_depth[node]) * der(C[node, :]) + C_nominal * der(_cross_section[node])) = (M[node, :] - M[node + 1, :]) / _dxq[node];
  end for;
