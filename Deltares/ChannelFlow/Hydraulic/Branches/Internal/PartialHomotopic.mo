...
equation
  ...
  for section in 2:n_level_nodes loop
    ...
    (if use_inertia then 1 else 0) * der(Q[section]) + g_n * (nominal_width[section] * nominal_depth[section]) * (H[section] - H[section - 1]) / dx - nominal_width[section] / density_water * _wind_stress + g_n * (theta * Q[section] * abs(Q[section]) + (1 - theta) * Q[section] * abs(Q_nominal)) / (friction_coefficient^2 * (nominal_width[section] * nominal_depth[section])^2 / (nominal_depth[section] * 2 + nominal_width[section])) = 0;
    // Substance transport
